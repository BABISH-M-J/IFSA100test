var commitApp = angular.module('commitApp', ['ui.bootstrap','textAngular','ngFileUpload','ngSignaturePad','ngSanitize', 'htmlSafe', 'ui.bootstrap.datetimepicker', 'ui.dateTimeInput', 'ui.mask']);
        
        commitApp.filter('apostraphe', function(){
            return function(input){
                return input ? input.replace(/&amp;#39;/g, "'") : '';
            }
        })
        
        commitApp.filter('ampersand', function(){
            return function(input){
                return input ? input.replace(/&amp;amp;/g, "&") : '';
            }
        })

        commitApp.filter('quote', function(){
            return function(input){
                return input ? input.replace(/&amp;quot;/g, '"') : '';
            }
        })

        commitApp.controller('commitController', function($scope, $modal, viewModel, urlService, sanitizer) {
            //datepicker
            ...
            $scope.viewAppItem = function (item) {
                portalRemotingMethods.getApplicationItemDetails(
                    item.id, 
                    function(result, event) {
                        $scope.loadAppItem(result, event);
                    }
                );
            }
        ...
        commitApp.controller('appItemErrorController', function ($scope, $modalInstance, data, Upload, $timeout, $sce, viewModel, $modal, sanitizer, $location) {
            //$scope.itemName = data;
        });
        commitApp.controller('appItemController', function ($scope, $modalInstance, data, Upload, $timeout, $sce, viewModel, urlService, $modal, sanitizer, apostrapheFilter, $filter) {
            $scope.data = data;
            ...
            if($scope.data.willHaveGroupTravel == 'Arrival Window')
            {
                $scope.data.travelOption = 'Independent';
            }
            else
            {
                $scope.data.travelOption = 'AdvantageDomesticAndInternational';
            }
            if($scope.data.recordType == "Travel_Plan")
            {
                try {
                    ...
                    $scope.data.arrivalDatePlus1 = angular.copy($scope.data.arrivalDate)
                    $scope.data.arrivalDatePlus1.setDate($scope.data.arrivalDatePlus1.getDate() + 1);
                    $scope.data.arrivalDatePlus1.setHours(0);
                    $scope.data.arrivalDatePlus1.setMinutes(0);   
                } catch (error) {
                    console.log(error);
                    $scope.data.arrivalDate = null;
                }
            }
            //$scope.data.travelOption = ($scope.data.willHaveGroupTravel != 'Arrival Instructions') ? 'AdvantageDomesticAndInternational' : 'Independent';
            ...
            if($scope.data.recordType == "Learning_Plan")
            {
                ...
                $scope.learningPlanDisclosureSet = $scope.data.learningPlanDisclosure != null;
            ...
            $scope.showCompletionNotification = function(showPopup) {
                if(!showPopup) {
                    studentCommitController.closeCompletionNotification($scope.viewModel.applicationId, function(result){});
                    var modalInstance = $modal.open({
                        animation: true,
                        size: 'lg',
        ...
        commitApp.controller('appItemsCompleteController', function($scope, $modalInstance) {
            $scope.ok = function () {
